class Program
{
    ...
    public static void Main(string[] args)
    {
        ...
        var temizlenmisSayilar = sayilar.Where(x => x != -1);
        var sonuc = temizlenmisSayilar.All(x => x >= 0);
        var ilkNotlar = temizlenmisSayilar.OrderBy(x => x).Take(2);
        foreach(var eleman in ilkNotlar)Console.WriteLine(eleman);
        ilkNotlar = temizlenmisSayilar.OrderByDescending(x => x).Take(2);
        foreach(var eleman in ilkNotlar)Console.WriteLine(eleman);
    }
}